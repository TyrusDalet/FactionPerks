...
--- Returns the highest active EEC perk tier for stat reporting.
...
--- Reports current EEC stat modifiers through framework/AAM interop.
local reportAAM = utils.makeAAMReporter(FACTION_DISPLAY_NAME, perkTable, getEECRank)
-- ============================================================
...
--- Returns the current Empire's Coffers rank, or nil if below P2.
--- @return number|nil rank Active Coffers tier from 2 to 4.
local function getEECCoffersRank()
    ...
    for _, actor in pairs(nearby.actors) do
        if types.NPC.objectIsInstance(actor) then
            ...
            if entry and entry.totalApplied > entry.baseBonus then
                ...
                utils.debug(2, "EEC", "Removed factor bonus " .. tostring(factorAmount)
    ...
    for _, actor in pairs(nearby.actors) do
        if types.NPC.objectIsInstance(actor) then
            ...
            if entry and entry.totalApplied > 0 then
                ...
                utils.debug(2, "EEC", "Restored " .. tostring(entry.totalApplied)
        ...
        if types.NPC.objectIsInstance(actor) then
            ...
            if entry and entry.appliedRank and entry.appliedRank < newRank then
                ...
                if delta > 0 then
                    ...
                    utils.debug(2, "EEC", "Upgraded " .. tostring(actor.id)
...
local function eecOnUiModeChanged(data)
    ...
    if not entry then
        ...
        utils.debug(2, "EEC", "First contact " .. tostring(npcId)
        ...
        if currentGold == entry.goldBeforeBonus then
            ...
            utils.debug(2, "EEC", "Restock " .. tostring(npcId)
                .. " baseBonus=" .. tostring(newBase)
                .. " totalApplied=" .. tostring(newTotal))
            ...
            if delta > 0 then
                ...
                utils.debug(2, "EEC", "Delta applied to " .. tostring(npcId)
                    .. " delta=+" .. tostring(delta)
                    .. " baseBonus=" .. tostring(newBase)
                    .. " totalApplied=" .. tostring(newTotal))
            ...
            elseif delta < 0 then
                ...
                entry.totalApplied = newTotal
                entry.appliedRank  = rank
                utils.debug(1, "EEC", "Negative delta corrected for " .. tostring(npcId)
                    .. " delta=" .. tostring(delta))
            end
        end
    end
...
local function onUpdate(dt)
    ...
    if eecFactorActive and not powerActive then
        ...
        utils.debug(1, "EEC", "Factor's Promise expired; restoring factor bonuses.")
        removeFactorBonuses()
    elseif not eecFactorActive and powerActive then
        ...
        utils.debug(1, "EEC", "Factor's Promise activated.")
    end
end
...
interfaces.ErnPerkFramework.registerPerk({
    ...
    category = {"FactionPerks", "Imperial Factions", "East Empire Company", 1},
...
interfaces.ErnPerkFramework.registerPerk({
    ...
    category = {"FactionPerks", "Imperial Factions", "East Empire Company", 2},
...
interfaces.ErnPerkFramework.registerPerk({
    ...
    category = {"FactionPerks", "Imperial Factions", "East Empire Company", 3},
...
interfaces.ErnPerkFramework.registerPerk({
    ...
    category = {"FactionPerks", "Imperial Factions", "East Empire Company", 4},
...
--                           on next dialogue
--  luaeec set <amount>
--                         - set the selected NPC's barter gold
