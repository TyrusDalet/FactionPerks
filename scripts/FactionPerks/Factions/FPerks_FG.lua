--[[

FG:
        FPerks_FG1_Passive          - +3 Strength, +3 Endurance, +10 Fortify Health,
                                      +5 Long Blade, +5 Blunt Weapon, +5 Axe
        FPerks_FG2_Passive          - +5 Strength, +5 Endurance, +20 Fortify Health,
                                      +10 Long Blade, +10 Blunt Weapon, +10 Axe
        FPerks_FG3_Passive          - +10 Strength, +10 Endurance, +35 Fortify Health,
                                      +18 Long Blade, +18 Blunt Weapon, +18 Axe
        FPerks_FG4_Passive          - +15 Strength, +15 Endurance, +50 Fortify Health,
                                      +25 Long Blade, +25 Blunt Weapon, +25 Axe
        FPerks_FG3_Enrage           - Power, Fortify Health 50pts, Fortify Fatigue 200pts,
                                      Fortify Attack 100pts, 30s duration.

    NOTE: Fortify Health is applied via stat.modifier so the maximum is raised correctly.
    appliedHealthMod is persisted via onSave/onLoad so the delta calculation is correct
    on reload and bonuses never stack. 
]]

local ns         = "FactionPerks"
local utils      = require("scripts.FactionPerks.utils")
local FactionGroupRank = utils.FactionGroupRank
local perkHidden  = utils.perkHidden
local safeAddSpell  = utils.safeAddSpell
local safeRemoveSpell = utils.safeRemoveSpell
local GUILD        = utils.FACTION_GROUPS.fightersGuild
local interfaces = require("openmw.interfaces")
local types      = require('openmw.types')
local self       = require('openmw.self')
local core       = require('openmw.core')
local ambient    = require('openmw.ambient')
local animation  = require('openmw.animation')
local async      = require('openmw.async')

local R = utils.requirements()

local perkTable = {
    [1] = { attributes = { endurance=3,  strength=3  }, skills = { longblade=5,  bluntweapon=5, axe=5  } },
    [2] = { attributes = { endurance=5,  strength=5  }, skills = { longblade=10, bluntweapon=10, axe=10 } },
    [3] = { attributes = { endurance=10, strength=10 }, skills = { longblade=18, bluntweapon=18, axe=18 } },
    [4] = { attributes = { endurance=15, strength=15 }, skills = { longblade=25, bluntweapon=25, axe=25 } },
}

local appliedStats = { attributes = {}, skills = {} }


local fg1_id = ns .. "_fg_dues_paid"
local fg2_id = ns .. "_fg_iron_discipline"
local fg3_id = ns .. "_fg_battle_tested"
local fg4_id = ns .. "_fg_champion_of_the_guild"

local setRank = utils.makeSetRank(perkTable, nil, appliedStats)

-- ============================================================
--  AAM INTEGRATION
--  Reports current active stat modifiers to AbilitiesAsModifiers
--  so they appear as a labelled source in attribute/skill tooltips.
--  Called after every setRank invocation.
-- ============================================================
local FACTION_DISPLAY_NAME = "Fighter's Guild Perks"

--- Returns the highest active Fighters Guild perk tier.
--- @return number rank Active tier from 0 to 4.
local function getFGRank()
    return utils.highestOwnedPerkRank({
        [1] = fg1_id,
        [2] = fg2_id,
        [3] = fg3_id,
        [4] = fg4_id,
    })
end

--- Reports current Fighters Guild stat modifiers through framework/AAM interop.
local reportAAM = utils.makeAAMReporter(FACTION_DISPLAY_NAME, perkTable, getFGRank)

-- ============================================================
--  FORTIFY HEALTH - stat.modifier with onSave/onLoad
-- ============================================================

local appliedHealthMod = 0

--- Applies Fighters Guild's Fortify Health modifier as a tracked replacement value.
--- @param value number Desired total health modifier from this faction.
local function applyHealthMod(value)
    local s = types.Actor.stats.dynamic.health(self)
    local delta = value - appliedHealthMod
    s.modifier = s.modifier + delta
    -- On respec (delta < 0), clamp current to the new lower cap.
    -- Without this, current can exceed base+modifier indefinitely.
    if delta < 0 then
        s.current = math.min(s.current, s.base + s.modifier)
    end
    appliedHealthMod = value
end

-- ============================================================
--  FIGHTERS GUILD COUNTER ATTACK - Iron Discipline (P2+)
-- ============================================================

local lastFGCounterTime = 0
local COUNTER_HIT_SOUND_DELAY = 0.2

local COUNTER_ATTACK_KEYS = {
    { start = "chop start",   stop = "chop hit"   },
    { start = "slash start",  stop = "slash hit"  },
    { start = "thrust start", stop = "thrust hit" },
}

local WEAPON_ANIMATION_GROUPS = {
    [types.Weapon.TYPE.ShortBladeOneHand] = "weapononehand",
    [types.Weapon.TYPE.LongBladeOneHand] = "weapononehand",
    [types.Weapon.TYPE.LongBladeTwoHand] = "weapontwohand",
    [types.Weapon.TYPE.BluntOneHand] = "weapononehand",
    [types.Weapon.TYPE.BluntTwoClose] = "weapontwohand",
    [types.Weapon.TYPE.BluntTwoWide] = "weapontwowide",
    [types.Weapon.TYPE.SpearTwoWide] = "weapontwowide",
    [types.Weapon.TYPE.AxeOneHand] = "weapononehand",
    [types.Weapon.TYPE.AxeTwoHand] = "weapontwohand",
}

--- Plays a short upper-body weapon flourish for direct counterattacks.
--- This is visual only; damage is still applied through the perk pipeline.
--- @param weapon GameObject Equipped weapon used to choose the animation group.
local function playCounterAttackAnimation(weapon)
    local controller = interfaces.AnimationController
    if not controller or not controller.playBlendedAnimation then
        return
    end
    if not weapon or not types.Weapon.objectIsInstance(weapon) then
        return
    end

    local group = WEAPON_ANIMATION_GROUPS[types.Weapon.record(weapon).type]
    if not group then
        return
    end

    local keys = COUNTER_ATTACK_KEYS[math.random(#COUNTER_ATTACK_KEYS)]
    local ok, err = pcall(controller.playBlendedAnimation, group, {
        startKey = keys.start,
        stopKey = keys.stop,
        priority = animation.PRIORITY.Weapon,
        blendMask = animation.BLEND_MASK.UpperBody,
        autoDisable = true,
        speed = 1.35,
    })
    if not ok then
        utils.debug(1, "FG", "Counterattack animation failed: " .. tostring(err))
    end
end

--- Lets SkillPerks Long Blade apply its own crit rules to this direct
--- counterattack when that mod/core is installed and the player is using a
--- Long Blade. Fighters Guild itself does not grant an inherent crit chance.
--- @param amount number Base counterattack damage.
--- @param weapon GameObject Equipped player weapon.
--- @return number amount Damage after optional external crit handling.
--- @return boolean critLanded Whether an external crit landed.
local function resolveExternalCounterCritical(amount, weapon)
    local longBlade = interfaces.SkillPerksLongBlade
    if not longBlade or not longBlade.resolveCounterCritical then
        return amount, false
    end

    local result = longBlade.resolveCounterCritical({
        amount = amount,
        weapon = weapon,
        sourceEffect = "FactionPerks_FG_CounterAttack",
        playSound = false,
    })
    if type(result) ~= "table" then
        return amount, false
    end
    return result.amount or amount, result.criticalLanded == true
end

--- Gives Long Blade Riposte first claim on shared counterattack timing.
--- If Riposte is ready, Fighters Guild does not also counterattack.
--- @param attack table Incoming attack payload.
--- @return boolean yield True if FG should skip its counterattack.
local function shouldYieldToRiposte(attack)
    local longBlade = interfaces.SkillPerksLongBlade
    if not longBlade or not longBlade.willAttemptRiposte then
        return false
    end
    return longBlade.willAttemptRiposte({
        attacker = attack.attacker,
        attack = attack,
        sourceEffect = "FactionPerks_FG_CounterAttack",
    }) == true
end

--- Chooses a vanilla armor hit sound based on the attacker's cuirass weight.
--- @param actor GameObject Actor receiving the counterattack sound.
--- @return string soundId Sound id to play.
local function getArmorHitSound(actor)
    local cuirass = types.Actor.getEquipment(actor, types.Actor.EQUIPMENT_SLOT.Cuirass)
    if cuirass and types.Armor.objectIsInstance(cuirass) then
        local weight = types.Armor.record(cuirass).weight
        if weight < 10 then
            return "light armor hit"
        elseif weight < 25 then
            return "medium armor hit"
        else
            return "heavy armor hit"
        end
    end
    return "light armor hit"
end

--- Plays the counterattack impact sounds after the flourish reaches its hit key.
--- @param actor GameObject Actor receiving the direct counterattack damage.
--- @param criticalLanded boolean Whether external crit handling landed a crit.
local function playCounterHitSoundDelayed(actor, criticalLanded)
    local armorSound = getArmorHitSound(actor)
    async:newUnsavableSimulationTimer(COUNTER_HIT_SOUND_DELAY, function()
        if criticalLanded then
            ambient.playSound("critical damage")
        end
        ambient.playSound(armorSound)
        ambient.playSound("health damage")
    end)
end

--- Estimates Fighters Guild counterattack damage from the player's weapon and stats.
--- @param weapon GameObject Equipped player weapon.
--- @param attacker GameObject Counterattacking actor, normally the player.
--- @return number damage Direct health damage to send to the missed attacker.
local function getCounterDamage(weapon, attacker)
    local rec = types.Weapon.record(weapon)
    local chop   = (rec.chopMinDamage   + rec.chopMaxDamage)   / 2
    local slash  = (rec.slashMinDamage  + rec.slashMaxDamage)  / 2
    local thrust = (rec.thrustMinDamage + rec.thrustMaxDamage) / 2
    local best   = math.max(chop, slash, thrust)
    local base   = best * (0.75 + math.random() * 0.5)
    local str       = types.Actor.stats.attributes.strength(attacker).modified
    local strFactor = 0.5 + 0.5 * (str / 100)
    local fatigue   = types.Actor.stats.dynamic.fatigue(attacker)
    local maxFat    = math.max(fatigue.base + fatigue.modifier, 1)
    local fatFactor = 0.75 + 0.25 * (fatigue.current / maxFat)
    return math.floor(base * strFactor * fatFactor)
end

interfaces.ErnPerkFramework.registerOnHitHandler({
    id = "FactionPerks_fighters_guild_counter_attack",
    priority = 300,
}, function(attack)
    if attack.successful             then return end
    if attack.attacker == self       then return end
    if not attack.weapon             then return end
    if not (attack.attacker and attack.attacker:isValid()) then return end
    if not R.hasPerk(fg2_id).check() then return end
    if shouldYieldToRiposte(attack)  then return end

    local cooldown = 10
    if     R.hasPerk(fg4_id).check() then cooldown = 1.5
    elseif R.hasPerk(fg3_id).check() then cooldown = 6
    end

    local now = core.getSimulationTime()
    if (now - lastFGCounterTime) < cooldown then return end

    local playerWeapon = types.Actor.getEquipment(self, types.Actor.EQUIPMENT_SLOT.CarriedRight)
    if not playerWeapon                               then return end
    if not types.Weapon.objectIsInstance(playerWeapon) then return end

    local dmg = getCounterDamage(playerWeapon, self)
    local critLanded = false
    dmg, critLanded = resolveExternalCounterCritical(dmg, playerWeapon)
    dmg = math.floor(dmg)
    if dmg <= 0 then return end

    playCounterAttackAnimation(playerWeapon)
    attack.attacker:sendEvent("FPerks_TakeDamage",
        utils.directHealthDamagePayload(dmg, self, "FactionPerks_FG_CounterAttack", "physical"))

    local fatigue = types.Actor.stats.dynamic.fatigue(self)
    fatigue.current = math.max(0, fatigue.current - 8)
    playCounterHitSoundDelayed(attack.attacker, critLanded)

    lastFGCounterTime = now
    utils.debug(2, "FG", "Counter Attack damage=" .. tostring(dmg) .. " crit=" .. tostring(critLanded))
end)

-- ============================================================
--  CONSOLE COMMANDS
--  luafg debug              - prints debug information
-- ============================================================

--- Handles Fighters Guild debug console commands.
--- @param mode string Console mode.
--- @param command string Raw console command.
local function onConsoleCommand(mode, command)
    local lower = utils.normalizeConsoleCommand(command):lower()

    if lower == "luafg debug" then
        local s = types.Actor.stats.dynamic.fatigue(self)
        print("Fatigue: base=" .. s.base .. " modifier=" .. s.modifier .. " current=" .. s.current)
    end
end

-- ============================================================
--  FIGHTERS GUILD PERKS
-- ============================================================

interfaces.ErnPerkFramework.registerPerk({
    id = fg1_id,
    localizedName = "Dues Paid",
    category = {"FactionPerks", "Imperial Factions", "Fighter's Guild", 1},
    localizedFlavour = "The basic drills are already sharpening your edge.",
    localizedDescription = "Grants the following stats: (+3 Strength, +3 Endurance, "
        .. "+10 Fortify Health, +5 Long Blade, +5 Blunt Weapon, +5 Axe)",
    hidden = perkHidden(GUILD, 0, 1),
    art = "textures\\levelup\\knight",
    cost = function() return utils.perkCost(1) end,
    requirements = {
        FactionGroupRank("fightersGuild",0),
        R.minimumLevel(1)
    },
    onAdd    = function()
        setRank(1)
        applyHealthMod(10)
        reportAAM()
        end,
    onRemove = function()
        setRank(nil)
        applyHealthMod(0)
        reportAAM()
        end,
})

interfaces.ErnPerkFramework.registerPerk({
    id = fg2_id,
    localizedName = "Iron Discipline",
    category = {"FactionPerks", "Imperial Factions", "Fighter's Guild", 2},
    localizedFlavour = "The Guild's contracts have hardened you. "
        .. "You wade into battle with the confidence of experience.",
    localizedDescription = "Effect 1: \n Grants the following stats: (+5 Strength, +5 Endurance, "
        .. "+20 Fortify Health, +10 Long Blade, +10 Blunt Weapon, +10 Axe)\f"
        .. "Effect 2: \n Counter Attack: When an enemy misses you with a weapon, "
        .. "you immediately strike back. 10s cooldown.",
    hidden = perkHidden(GUILD, 3, 5),
    art = "textures\\levelup\\knight",
    cost = function() return utils.perkCost(2) end,
    requirements = {
        R.hasPerk(fg1_id),
        FactionGroupRank("fightersGuild",3),
        R.minimumAttributeLevel('strength', 40),
        R.minimumLevel(5),
    },
    onAdd    = function()
        setRank(2)
        applyHealthMod(20)
        reportAAM()
        end,
    onRemove = function()
        setRank(nil)
        applyHealthMod(0)
        reportAAM()
        end,
})

interfaces.ErnPerkFramework.registerPerk({
    id = fg3_id,
    localizedName = "Battle Tested",
    category = {"FactionPerks", "Imperial Factions", "Fighter's Guild", 3},
    localizedFlavour = "Daedra, bandits, necromancers - you have killed them all on contract. "
        .. "When the moment demands it, you can call upon a terrifying fury.",
    localizedDescription = "Effect 1: \n Grants the following stats: (+10 Strength, +10 Endurance, "
        .. "+35 Fortify Health, +18 Long Blade, +18 Blunt Weapon, +18 Axe)\f"
        .. "Effect 2: \n Grants Martial Rage (1/day): Fortify Health 50pts, Fortify Fatigue 200pts, "
        .. "Fortify Attack 100pts for 30s.\f"
        .. "Effect 3: \n Counter Attack cooldown reduced to 6s.",
    hidden = perkHidden(GUILD, 6, 10),
    art = "textures\\levelup\\knight",
    cost = function() return utils.perkCost(3) end,
    requirements = {
        R.hasPerk(fg2_id),
        FactionGroupRank("fightersGuild",6),
        R.minimumAttributeLevel('strength', 50),
        R.minimumLevel(10),
    },
    onAdd = function()
        setRank(3)
        applyHealthMod(35)
        reportAAM()
        safeAddSpell("FPerks_FG3_Enrage")
    end,
    onRemove = function()
        setRank(nil)
        applyHealthMod(0)
        reportAAM()
        safeRemoveSpell("FPerks_FG3_Enrage")
    end,
})

interfaces.ErnPerkFramework.registerPerk({
    id = fg4_id,
    localizedName = "Champion of the Guild",
    category = {"FactionPerks", "Imperial Factions", "Fighter's Guild", 4},
    localizedFlavour = "The Fighters Guild holds you as one of its finest. "
        .. "Your counter attack is now almost instantaneous.",
    localizedDescription = "Effect 1: \n Grants the following stats: (+15 Strength, +15 Endurance, "
        .. "+50 Fortify Health, +25 Long Blade, +25 Blunt Weapon, +25 Axe)\f"
        .. "Effect 2: \n Counter Attack cooldown reduced to 1.5s.",
    hidden = utils.leaderTrainingHidden("fightersGuild", perkHidden(GUILD, 9, 15)),
    art = "textures\\levelup\\knight",
    cost = function() return utils.perkCost(4) end,
    requirements = {
        utils.leaderTrainingRequirement("fightersGuild"),
        utils.leaderTrainingRankRequirement("fightersGuild", 9),
        R.hasPerk(fg3_id),
        FactionGroupRank("fightersGuild",9),
        R.minimumAttributeLevel('strength', 75),
        R.minimumLevel(15),
    },
    onAdd    = function()
        setRank(4)
        applyHealthMod(50)
        reportAAM()
        end,
    onRemove = function()
        setRank(nil)
        applyHealthMod(0)
        reportAAM()
        end,
})

-- ============================================================
--  SAVE / LOAD
-- ============================================================

local function onSave()
    return {
        appliedHealthMod = appliedHealthMod,
        appliedStats = appliedStats
    }
end

local function onLoad(data)
    data = data or {}
    appliedHealthMod = data.appliedHealthMod or 0
    utils.clearSavedAppliedStats(data.appliedStats, appliedStats)
end

return {
    engineHandlers = {
        onSave = onSave,
        onLoad = onLoad,
        onConsoleCommand = onConsoleCommand,
    }
}
